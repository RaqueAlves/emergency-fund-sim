...
class CDBHibridoStrategy(RegressiveTaxStrategy):
    def __init__(self, percentage_of_cdi: float, anual_rate: float):
        ...
        self.__ipca_rate = 0.05

    def calculate_gross_amount(self, principal: float, days: int) -> float:
        return principal * (1 + self.__anual_rate + self.__ipca_rate) ** (days / 252)
    ...
    def get_profitability_description(self) -> str:
        '''Retorna uma string legível, ex: '100% do CDI + IPCA + 5% a.a.'''
        return f'{self.__percentage_of_cdi * 100:.0f}% do CDI + IPCA + {self.__anual_rate * 100:.0f}% a.a.'